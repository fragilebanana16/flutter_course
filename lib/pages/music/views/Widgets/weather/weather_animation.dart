...
class WeatherAnimationState extends State<WeatherAnimation> {
  Future<Null> _loadAssets() async {
    ...
    await _images?.load(<String>[
      'assets/images/weather/clouds-0.png',
      'assets/images/weather/clouds-1.png',
      'assets/images/weather/weathersprites.png',
    ]);
    ui.Image? image =
        _images?.getImage('assets/images/weather/weathersprites.png');
    // Load the sprite sheet, which contains snowflakes and rain drops.
    String json = await DefaultAssetBundle.of(context)
        .loadString('assets/images/weather/weathersprites.json');
    _sprites = SpriteSheet(image: image!, jsonDefinition: json);
  }
...
const List<Color> _kBackgroundColorsTop = const <Color>[
  const Color(0xff5ebbd5), //unknown
...
const List<Color> _kBackgroundColorsBottom = const <Color>[
  const Color(0xff4aaafb), //unknown
...
class WeatherWorld extends NodeWithSize {
  ...
  WeatherWorld() : super(const Size(2048.0, 2048.0)) {
    ...
    _cloudsSharp = CloudLayer(
        image: _images!.getImage('assets/images/weather/clouds-0.png')!,
        rotated: false,
        dark: false,
        loopTime: 90.0);
    ...
    _cloudsDark = CloudLayer(
        image: _images!.getImage('assets/images/weather/clouds-1.png')!,
        rotated: true,
        dark: true,
        loopTime: 120.0);
...
class CloudLayer extends Node {
  CloudLayer(
    ...
    motions.run(MotionRepeatForever(
