...
import 'package:flutter_course/pages/toolBox/views/invididualViews/file_share/ui/common_view/empty_widget.dart';
import 'package:flutter_course/pages/toolBox/views/invididualViews/file_share/ui/common_view/square_icon_button.dart';
import 'package:flutter_course/pages/toolBox/views/invididualViews/file_share/ui/list_file/file_tile_client.dart';
...
class _ListSharedFilesState extends State<ListSharedFiles> {
  ...
  _genArrayFileTile(
          List<SharedFile> oldFiles, List<SharedFile> filteredFiles) =>
      List<FileTileClient>.generate(filteredFiles.length, (int index) {
        return FileTileClient(
          ...
          onPressed: () {
            final oldIndex = oldFiles.indexWhere(
                (f) => f.name == filteredFiles.elementAt(index).name);
            final contentSize =
                mainListScrollController.position.viewportDimension +
      ...
      children: [
        ...
        Container(
          ...
          child: SquareIconButton(
            icon: Icons.sync,
            onPressed: () async {
              final files = (await fileRepository.getSharedFilesWithState())
                  .getOrElse(() => {});
              if (mounted) {
                context
                    .read<FileProvider>()
                    .addAllSharedFiles(sharedFiles: files);
              }
            },
          ),
        ),
