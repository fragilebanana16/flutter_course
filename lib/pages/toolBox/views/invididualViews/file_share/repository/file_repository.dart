...
import 'package:flutter_course/pages/toolBox/views/invididualViews/file_share/entity/api_error.dart';
...
class FileRepository {
  ...
  Future<Either<ApiError, Set<SharedFile>>> getSharedFilesWithState() async {
    ...
    if (originalFiles.isLeft()) {
      return Left(
          originalFiles.fold((l) => l, (r) => throw UnimplementedError()));
    }
    ...
    String connectedAddress =
        'http://${getIt.get<GlobalScopeData>().connectedIPAddress}';

    final newFileList = await Future.wait(originalFilesRight.map((file) async {
      // check exist in Hive
      final savedAvailableFile =
          await getIt.get<SharedFileClient>().get(file.name);
      ...
      final savedFile =
          File('${savedAvailableFile.savedDir}/${savedAvailableFile.name!}');
